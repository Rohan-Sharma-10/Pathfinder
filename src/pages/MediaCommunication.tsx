import { mediaDegreesData } from '@/data/Media';
import { ArrowLeft, Camera, TrendingUp, Users, Target } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { CourseCard } from "@/components/Course";

export default function MediaCommunication() {
  const totalDegrees = mediaDegreesData.length;
  const highDemandDegrees = mediaDegreesData.filter(d => d.marketDemand.level === 'High').length;
  const avgSalaryRange = {
    min: Math.min(...mediaDegreesData.map(d => d.salaryProgression.entryLevel)),
    max: Math.max(...mediaDegreesData.map(d => d.salaryProgression.senior))
  };
        ...
        <div className="relative container mx-auto px-4 py-16">
          ...
          <div className="max-w-4xl mx-auto text-center space-y-6">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-foreground leading-tight">
              ...
              <span className="block text-primary mt-2">Media & Communication</span>
            </h1>
            
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Explore dynamic career opportunities in journalism, mass communication, film production, and digital media. 
              Shape the future of communication and storytelling with comprehensive industry insights.
            </p>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-foreground mb-4">
              Comprehensive Media Programs
            </h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              Each program includes detailed market analysis, career progression insights, and realistic salary expectations 
              to help you make informed decisions about your media and communication career.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {mediaDegreesData.map((degree) => (
